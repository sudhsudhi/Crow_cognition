...
class Shapy(Widget):
    ...
    def __init__(self, **kwargs):
        ...
        self.cir_center_x=0
        self.cir_center_y=0
        self.trial_list=[3,3,3,3,2,2,2,2] #number of trials left of each shape(i.e of the sound associated with that sound), 0th for circle, 1st for ellipse, 2nd for rectangle, 3rd for triangle, 4th,55h,6th,7th for incorrect snd1,2,3,4 respectively.
        self.sound_shape=-1 # will be changed by positioner. will be 0 if sound for circle is played. 1 for ellipse. 2 for rectangel.3 for triangle. 4 ,5,6,7 for incorrect snd 1,2,3,4 respectively.
    ...
    def positioner(self):  #makes sure that there is atleast one shape in each of the 4 sectors. places the shape randomly within the sector.also plays a sound and edits self.trial_list and self.sound_shape
        ...
        l = random.randint(0,7) #gives 0/1/2/3...7
        ...
        pp=['CIRCLE','ELLIPSE','RECTANGLE','TRIANGLE','incorrect_snd1','incorrect_snd2','incorrect_snd3','incorrect_snd4']
        ...
        if self.sound_shape==4:
          winsound.PlaySound('incorrect_snd1.wav', winsound.SND_FILENAME | winsound.SND_ASYNC)   # winsound.SND_ASYNC makes sure sound is asynchronous with the program. That is program doesn't block till sound is fully played.
        if self.sound_shape==5:
          winsound.PlaySound('incorrect_snd2.wav', winsound.SND_FILENAME | winsound.SND_ASYNC)   # winsound.SND_ASYNC makes sure sound is asynchronous with the program. That is program doesn't block till sound is fully played.
        if self.sound_shape==6:
          winsound.PlaySound('incorrect_snd3.wav', winsound.SND_FILENAME | winsound.SND_ASYNC)   # winsound.SND_ASYNC makes sure sound is asynchronous with the program. That is program doesn't block till sound is fully played.
        if self.sound_shape==7:
          winsound.PlaySound('incorrect_snd4.wav', winsound.SND_FILENAME | winsound.SND_ASYNC)   # winsound.SND_ASYNC makes sure sound is asynchronous with the program. That is program doesn't block till sound is fully played.
    ...
    def on_touch_down(self, touch):   #self refers to the widget, touch to the the tough motion event#on_touch_down is the function called by kivy when you touch. It's name is standard(not variable)
          ...
          if  self.nextt =='o' and not(self.blank):                      
                print "self.inside_out(touch): ", str(self.inside_out(touch)) ," (0,1,2,3 for peck within circle,ellipse,rectangle,triangle respectively. -1 if touched outside all the shapes.)"

                if self.inside_out(touch)!=-1:      #i.e touched within one of the shapes

                    print 'pecked_inside_shape'
...
if __name__ == '__main__':
    Config.set('graphics','borderless', 0)   #not necessary all the time, but sometimes required to prevent window from full-screening(tht is from boders from disappearing).
