...
class Shapy(Widget):
    ...
    def __init__(self, **kwargs):
        ...
        self.canvas_shape=4        #0= Circle, 1= Ellipse,2= Rectangle, 3=Triangle ,4=starting state
        self.trial_list=[[3,2],[3,2],[3,2],[3,2]] #0 th element of self.trial_list indicates trials of circle; 0th element of 0th list is number of incorrect trials of circle left; 1st element of 0th list is number of correct trials of circle left;
                                                  #similarly for other shapes also.
        self.trial_state = 0 #set in positioner #for the purpose of opening and closing of door. 1 in case door has to be opened(correct trial), 0 in case door need not be closed(incorrect trial). It is an int.

    
    def positioner(self):   #randomly places the shape anywhere and plays the sound randomly based on self.trial_list and self.canvas_shape. Also generates a random shape. Also takes care that 3 correct and 2 incorrect trials of each shape are performed.

        k = random.randint(0,3) #gives 0/1/2/3
        while self.trial_list[k][0]+ self.trial_list[k][1] == 0 :# when incorrect+correct trilas left = 0
            k = random.randint(0,3)
        self.canvas_shape=k   #now the shape is decided

        l = random.randint(0,1) #gives 0/1
        while self.trial_list[self.canvas_shape][l]== 0 : # to check correct/incorrect trials are not yet finished
            l = random.randint(0,1)
        #if l = 0 incorrect trial, or if l=1 correct trial
        self.trial_state=l
        self.trial_list[self.canvas_shape][l]-=1
        

        pp=['CIRCLE','ELLIPSE','RECTANGLE','TRIANGLE']
        kk=['INCORRECT',"CORRECT"]

        print "CURRENT SHAPE: "+ pp[self.canvas_shape]," correct/incorrect: "+ kk[self.trial_state]
        ...
        if self.canvas_shape==0:
                ...
                if self.trial_state ==1:
                    ...
                    #incorrect trial
                    p=['rectangle_hawk.wav','ellipse_dog.wav','triangle_lion.wav']

                    pi=random.randint(0,2)

                    winsound.PlaySound(p[pi], winsound.SND_FILENAME | winsound.SND_ASYNC)   # winsound.SND_ASYNC makes sure sound is asynchronous with the program. That is program doesn't block till sound is fully played.
                


        if self.canvas_shape==1:
                #ELLIPSE
                self.area =  40000                  #it is here just to initialize (there is no need of updating, but it's still here)
                ...
                if self.trial_state ==1:
                    ...
                    #incorrect_trial
                    p=['rectangle_hawk.wav','circle_crow.wav','triangle_lion.wav']

                    pi=random.randint(0,2)

                    winsound.PlaySound(p[pi], winsound.SND_FILENAME | winsound.SND_ASYNC)   # winsound.SND_ASYNC makes sure sound is asynchronous with the program. That is program doesn't block till sound is fully played.

        if self.canvas_shape==2:
                ...
                if self.trial_state ==1:
                    ...
                    #incorrect trial
                    p=['ellipse_dog.wav','circle_crow.wav','triangle_lion.wav']

                    pi=random.randint(0,2)

                    winsound.PlaySound(p[pi], winsound.SND_FILENAME | winsound.SND_ASYNC)   # winsound.SND_ASYNC makes sure sound is asynchronous with the program. That is program doesn't block till sound is fully played.

        if self.canvas_shape==3:
                ...
                if self.trial_state ==1:
                    ...
                    #incorrect trial
                    p=['ellipse_dog.wav','circle_crow.wav','rectangle_hawk.wav']
    ...
    def on_touch_down(self, touch):   #self refers to the widget, touch to the the tough motion event#on_touch_down is the function called by kivy when you touch. It's name is standard(not variable)
          ...
          pp=['CIRCLE','ELLIPSE','RECTANGLE','TRIANGLE']
          self.sheet.cell(row= self.peck_no+2,column=5).value = str(pp[self.canvas_shape]) #shape
          kk=['INCORRECT',"CORRECT"]
          self.sheet.cell(row= self.peck_no+2,column=6).value = str(kk[self.trial_state]) #correct/incorrect trial
    ...
    def update(self,dt):        #just check why is it necesaary to add dt, it is not working without dt
        ...
        if self.nextt=='start':
                ...
                self.sheet.cell(row= 1,column=5).value = 'Shape'
                self.sheet.cell(row= 1,column=6).value = 'Correct/Incorrect_trial'
