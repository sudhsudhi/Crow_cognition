...
class Shapy(Widget):
    ...
    def update(self,dt):        #just check why is it necesaary to add dt, it is not working without dt
        ...
        with self.canvas:
            ...
            Triangle(points=(self.center_x, self.center_y + self.circum_radius , self.center_x - 0.866025*self.circum_radius , self.center_y -0.5*self.circum_radius  , self.center_x +0.866025*self.circum_radius , self.center_y -0.5*self.circum_radius,))
