...
class Circly(Widget):
    ...
    def __init__(self, **kwargs):
        ...
        nextt != 'o' for 5 s ( or 't' seconds) after which the crow pecks within the shape. In this time nextt will become a datetime.datetime object and indicates the time(absolute time at whic screen should reappear).
    ...
    def play_sound(self):
        winsound.PlaySound('crow_caw.wav', winsound.SND_FILENAME)   #works only with wav file , keep this line at the end only or else the circle will disappear only after the sound plays
    ...
    def on_touch_down(self, touch):   #self refers to the widget, touch to the the tough motion event#on_touch_down is the function called by kivy when you touch. It's name is standard(not variable)
          ...
          if  self.nextt =='o' and not(self.blank):                      
                ...
                if xc1 +yc1 <= (self.radius)**2:      #make sure the size here is same as that in kv file
                    ...
                    t= 5  #number of seconds after which , circle reappears.
